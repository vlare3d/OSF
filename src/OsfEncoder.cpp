...
void osfEncoder::setConfig(OsfConfig config)
{
    ...
    _config.baseLiftingDelayBefore = config.baseLiftingDelayBefore * 1000; //ms
    _config.baseLiftingDelayAfter = config.baseLiftingDelayAfter * 1000; //ms
    _config.baseRetractDelay = config.baseRetractDelay * 1000; //ms
    ...
    _config.baseLiftingSpeedEnd = config.baseLiftingSpeedEnd;// mm/min
    _config.baseLiftingCurvature = config.baseLiftingCurvature;
    _config.baseLiftingCurvatureDec = config.baseLiftingCurvatureDec;
    ...
    _config.liftingSpeedEnd = config.liftingSpeedEnd;
    _config.liftingCurvature = config.liftingCurvature;
    _config.liftingCurvatureDec = config.liftingCurvatureDec;
    ...
    _config.baseRetractSpeedEnd = config.baseRetractSpeedEnd;
    _config.baseRetractCurvature = config.baseRetractCurvature;
    _config.baseRetractCurvatureDec = config.baseRetractCurvatureDec;
    ...
    _config.retractSpeedEnd = config.retractSpeedEnd;
    _config.retractCurvature = config.retractCurvature;
    _config.retractCurvatureDec = config.retractCurvatureDec;
...
/**
    *用于将给定的图片灰度数据及支撑数据压缩至点集
    *@param data 当前层灰度数据
    *@param dataSupport 当前层支撑数据
    *@param id 当前切片层数
    **/
...
void osfEncoder::saveOsfFile(const std::string& filename)
{
    ...
    of2B(out, static_cast<short>(_config.baseLiftingSpeedEnd));
    out << (unsigned char)(_config.baseLiftingCurvatureDec);
    of2B(out, static_cast<short>(_config.liftingSpeedEnd));
    out << (unsigned char)(_config.liftingCurvatureDec);
    of2B(out, static_cast<short>(_config.baseRetractSpeedEnd));
    out << (unsigned char)(_config.baseRetractCurvatureDec);
    of2B(out, static_cast<short>(_config.retractSpeedEnd));
    out << (unsigned char)(_config.retractCurvatureDec);
    of2B(out, static_cast<short>(_config.baseLiftingDelayBefore / 10));
    of2B(out, static_cast<short>(_config.baseLiftingDelayAfter / 10));
    of2B(out, static_cast<short>(_config.baseRetractDelay / 10));
...
/**
    *将压缩好的点集写入至输出流中
    *@param out 输出流
    *@param htPointSets 压缩后的点集
    *@param out headType 图片头标志 用于判断是否需要单独曝光支撑
    **/
...
/**
    *用于将给定的图片灰度数据转换成点集后存入vector中（具体压缩方式见协议）
    *@param data 当前层灰度数据
    *@param pointSets 当前层点集vector
    **/
